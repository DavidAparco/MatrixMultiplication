/* cannon.c -- Usa el algoritmo de Fox para multiplicar dos matrices cuadradas
 La raíz cuadrada del número de procesadores debe ser divisible por el orden de las matrices */
#include <stdio.h>
#include "mpi.h"
#include <math.h>
#include <stdlib.h>
...
main(int argc, char* argv[]) {
    ...
    Print_matrix("Se leyó A =", local_A, &grid, n);

    local_B = Local_matrix_allocate(n_bar);
    Order(local_B) = n_bar;
    Read_matrix("Ingrese B ", local_B, &grid, n);
    Print_matrix("Se leyó  B =", local_B, &grid, n);
	...
	Print_matrix("El producto es C : ", local_C, &grid, n);
